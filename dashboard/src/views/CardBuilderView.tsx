...
export default function CardBuilderView() {
  ...
  const { data: card, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return res.data  // never touch state here
    },
  })

  // Source of truth — local during drag, server otherwise
  ...
  const handleDragStart = (index: number) => {
    ...
    if (!localSegments && card) {
      setLocalSegments([...card.segments])
    }
  }
  ...
  const handleDrop = () => {
    ...
    if (localSegments) {
      reorderMutation.mutate(localSegments.map(s => s.id))
    }
  }
